...
function runBenchmark(name, iterations, fn) {
  ...
  return { name, iterations, stats, opsPerSec };
}

console.log('='.repeat(70));
console.log('  krusch-pre-router Latency Benchmark (10,000 iterations)');
console.log('='.repeat(70));

const coldBench = runBenchmark('Cold Regex Heuristic (classifyPreRoute)', 10000, (p) => {
  classifyPreRoute(p);
});
...
const warmBench = runBenchmark('Warm LRU Cache Hit (router.classify)', 10000, (p) => {
  router.classify(p);
});
...
console.log('\n' + '='.repeat(70));
console.log('Summary:');
console.log(`Cold Heuristic: ${coldBench.stats.p50.toFixed(1)} µs p50 | Warm Cache Hit: ${warmBench.stats.p50.toFixed(1)} µs p50`);
console.log('='.repeat(70) + '\n');
